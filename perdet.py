...
def DisplayDiseasePercentage(Disease):
    ...
    if Percent>90:
          messagebox.showinfo("leaf", str(round(Percent, 2)) + "% Tomato Bactrial spot Plant  Diseased Leaf")
    else:
        if Percent > 40:
            messagebox.showinfo("leaf", str(round(Percent, 2)) + "% Tomato Early spot Plant Diseased Leaf")
        else:
            if Percent > 30:
                messagebox.showinfo("leaf", str(round(Percent, 2)) + "% Tomato Target spot Plant Diseased Leaf")
            else:
                if Percent > 20:
                    messagebox.showinfo("leaf", str(round(Percent, 2)) + "% Tomato Bactrial Late blight Plant  Diseased Leaf")
                else:
                     messagebox.showinfo("Plant", "Healthy")
Alpha = None
